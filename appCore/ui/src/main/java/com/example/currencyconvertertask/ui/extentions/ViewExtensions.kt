...
fun TextInputLayout.showError(message: String, showKeyboard: Boolean = true) {
    error = message
    isErrorEnabled = true
    if (showKeyboard) {
        requestFocus()
...
fun Drawable.setTintColor(@ColorInt tintColor: Int): Drawable {
    DrawableCompat.wrap(this)
    DrawableCompat.setTint(this.mutate(), tintColor)
...
fun View.expand() {
    val matchParentMeasureSpec = View.MeasureSpec.makeMeasureSpec(
        (this.parent as View).width, View.MeasureSpec.EXACTLY
    )
...
fun ImageView.loadImage(imageUrl: String? = null) {
    if (imageUrl != null && imageUrl.isNotEmpty() ) {
        ...
        setImageResource(R.drawable.icon_place_holder)
    }
}

fun ImageView.loadImageGif(raw: Int? = null) {
    if ( raw != null) {
        Glide.with(context)
            .asGif()
            .load( raw)
